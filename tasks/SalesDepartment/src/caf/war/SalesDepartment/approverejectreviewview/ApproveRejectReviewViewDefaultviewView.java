 ...
 * 
 */
package caf.war.SalesDepartment.approverejectreviewview;
...
@ManagedBean(name = "ApproveRejectReviewViewDefaultviewView")
@SessionScoped
@ExpireWithPageFlow
@DTManagedBean(displayName = "ApproveRejectReviewView/default", beanType = BeanType.PAGE)
public class ApproveRejectReviewViewDefaultviewView extends com.webmethods.caf.faces.bean.task.BaseTaskDetailsPageBean {
	...
	private static final String[][] TASKDISPLAYPROVIDER_PROPERTY_BINDINGS = new String[][] {
		{"#{TaskDisplayProvider.taskID}", "#{ApproveRejectReviewView.taskID}"},
	};
	...
	public String cancelView() {
		try {
			...
			String url = getApproveRejectReviewView().getFinishUrl();
	...
	public String completeTask() {
		try {
			if( !getApproveRejectReview().isUpdateable() ){
			...
			// do the work
			getApproveRejectReview().completeTask(); 
	...
	public String submitTask() {
		try {
			...
			// do the work
			getApproveRejectReview().applyChanges();
		} catch (Exception e) {
			error(e);
			log(e);
		}
	...
	public String assignToPrincipals() {
		try {
			...
			TaskContentProvider tp = getApproveRejectReview();
	...
	private static final String[][] INITIALIZE_PROPERTY_BINDINGS = new String[][] {
		{"#{ApproveRejectReview.reset}", null}
	};
	private transient caf.war.SalesDepartment.approverejectreviewview.ApproveRejectReviewView approveRejectReviewView = null;
	...
	private static final String[][] APPROVEREJECTREVIEW_PROPERTY_BINDINGS = new String[][] {
		{"#{ApproveRejectReview.taskID}", "#{ApproveRejectReviewView.taskID}"},
		{"#{ApproveRejectReview.autoAccept}", "false"},
		{"#{ApproveRejectReview.adhocRouting}", "false"},
	};
	...
	protected void beforeRenderResponse() {
		...
		try {
			getApproveRejectReview().refresh();
		} catch( Exception ex ) {
			error(ex);
			log(ex);
		}
	...
	public caf.war.SalesDepartment.approverejectreviewview.ApproveRejectReviewView getApproveRejectReviewView()  {
		if (approveRejectReviewView == null) {
		    approveRejectReviewView = (caf.war.SalesDepartment.approverejectreviewview.ApproveRejectReviewView)resolveExpression("#{ApproveRejectReviewView}");
		}
		return approveRejectReviewView;
	}
